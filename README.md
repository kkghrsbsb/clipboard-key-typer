...
程序会在输入 `1` 后、倒计时开始前读取并缓存剪切板内容，避免目标网页在获得焦点后改写剪切板。默认使用慢速输入：字符间隔约 `80ms ~ 180ms`，每次按键按下时间约 `18ms ~ 42ms`，换行后暂停约 `350ms ~ 650ms`。每次任务开始前会显示读取到的剪切板字节数/字符数，方便判断是剪切板读取问题还是目标输入框拦截问题。
